...
## Что такое технические входные ограничения

В конфиге ноды есть блок `input_control`. Это не бизнес-валидация, а только технические правила ingestion:
- нужно ли подавлять точные дубликаты (`suppress_duplicates`);
- применять ли deadband;

По умолчанию сервис ничего не режет:
...
Блок `value_transform` нужен для технической адаптации значения:
- `scale_factor`
- `offset`
- `target_unit`
...
`make up` поднимает полностью контейнерный стенд:
- `opcua-client`
- `mock-opcua-server`
- `redis`
- `rabbitmq`
